...
const ProfileService = {
    ...
    async fetchZipCodeData(postalCode, miles) {
        const options = {
            method: 'GET',
            headers: {
        ...
        try {
            const zipCodesObjs = await fetch(`${defaultConfig.REDLINE_API.APP_URI}/rest/radius.json/${postalCode}/${miles}/mile`, options);
    ...
    async basicUserSearchQuery(userId, searchParams) {
        try {
    ...
    async advancedUserSearchQuery(userId, searchParams) {
        try {
            const userWhoHaveNotBlockedYou = {
                'blockedUsers.users.userId': {
                    $not: { $eq: new Types.ObjectId(userId) },
    ...
    async sendMessageToUser(sender, receiverOfMessage, senderImageSource, recieverImageSource, message) {
        try {
