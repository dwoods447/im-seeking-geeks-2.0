...
const ProfileController = {
    ...
    async addUserToFavorites(req, res, next) {
        ...
        if (!userToAdd)
            return res.status(422).json({ message: 'Unable to locate this account' });
        const userIsFavorited = await UserService.checkIfUserIsAlreadyInFavorites(currentUser, userToAdd);
        if (userIsFavorited)
            return res.status(400).json({ message: 'User is already in your list of favorites' });
    ...
    async removeUserFromFavorites(req, res, next) {
        ...
        if (!personRemoved)
            return res.status(400).json({ message: 'User could not be removed!' });
    ...
    async addUserToBlockList(req, res, next) {
        ...
        if (userIsBlocked)
            return res.status(400).json({ message: 'User is already in the list' });
    ...
    async removeUserFromBlockList(req, res, next) {
        ...
        if (!personToUnBlock)
            return res.status(422).json({ message: 'Error retrieving users account' });
        const userUnBlocked = await UserService.removeUserFromBlockList(blocker, personToUnBlock);
        if (!userUnBlocked)
            return res.status(422).json({ message: 'Error removing user from blockList' });
    ...
    async retrieveProfile(req, res, next) {
        ...
        if (!targetUser)
            return res.status(422).json({ message: 'User profile could not be found' });
        // Update age
        ...
        if (req.body.userId !== req.userId)
            await UserService.addProfileViewer(targetUser, currentUser);
        return res.status(200).json({ message: 'User found', user: targetUser, blocked: false });
    },
    async basicProfileSearch(req, res, next) {
        const currentUser = await UserService.checkIfUserLoggedIn(req.userId);
        if (!currentUser)
            return res.status(401).json({ message: 'Unauthorized you are not logged in!' });
        ...
        if (searchParams.gender)
            requestParams.gender = searchParams.gender;
        ...
        if (searchParams.ethnicity)
            requestParams.ethnicity = { $in: searchParams?.ethnicity };
        if (searchParams.bodyType)
            requestParams.bodyType = searchParams.bodyType;
        if (searchParams.datingIntent)
            requestParams.datingIntent = searchParams.datingIntent;
        if (searchParams.highestEducation)
            requestParams.highestEducation = searchParams.highestEducation;
        if (searchParams.onlineStatus)
            requestParams.onlineStatus = searchParams.onlineStatus;
        if (searchParams.city)
            requestParams.city = searchParams.city;
        if (searchParams.usState)
            requestParams.state = searchParams.usState;
        if (searchParams.usState)
            requestParams.state = searchParams.usState;
        ...
        if (!zipCodes)
            return res.status(404).json({ message: 'No zipcodes returned!' });
        if (zipCodes.error_code === 404)
            return res.status(404).json({ message: 'You entered an invalid zipcode' });
        if (searchParams.postalCode)
            requestParams.postalCode = { $in: zipCodes };
    ...
    async advancedProfileSearch(req, res, next) {
        const currentUser = await UserService.checkIfUserLoggedIn(req.userId);
        if (!currentUser)
            return res.status(401).json({ message: 'Unauthorized you are not logged in!' });
        // Get user search parameters
        ...
        if (searchParams?.gender)
            requestParams.gender = requestParams.gender;
        if (searchParams?.minAge || searchParams?.maxAge)
            requestParams.age = {
                $gte: Number.parseInt(searchParams?.minAge),
                $lte: Number.parseInt(searchParams.maxAge),
        ...
        if (searchParams.usState)
            requestParams.state = searchParams.usState;
        if (searchParams.martialStatus)
            requestParams.maritalStatus = { $in: searchParams.maritalStatus };
        if (searchParams.hasChildren)
            requestParams.hasChildren = searchParams.hasChildren;
        if (searchParams.doesSmoke)
            requestParams.doesSmoke = searchParams.doesSmoke;
        if (searchParams.doesDoDrugs)
            requestParams.doesDoDrugs = searchParams.doesDoDrugs;
        if (searchParams.doesDrink)
            requestParams.doesDrink = searchParams.doesDrink;
        if (searchParams.religion.length > 0)
            requestParams.religion = { $in: searchParams.religion };
        if (searchParams.doesHavePets)
            requestParams.doesHavePets = searchParams.doesHavePets;
        if (searchParams.income)
            requestParams.income = {
                $gte: Number.parseInt(searchParams.income),
            };
        if (searchParams.doesDateInteracially)
            requestParams.doesDateInteracially = searchParams.doesDateInteracially;
    ...
    async updateExtendedUserProfile(req, res, next) {
        const { seekingGenders, height, relationshipTypeSeeking, hairColor, eyeColor, highestEducation, secondLanguage, bodyType, postalCode, usState, city, maritalStatus, hasChildren, description, doesSmoke, doesDoDrugs, doesDrink, religion, profession, doesHavePets, personality, ambitiousness, datingIntent, longestRelationShip, income, doesDateInteracially, interacialDatingPreferences, raceDatingPreferences, geekInterests, selectedMaritalStatuses, } = req.body;
        const user = await UserService.checkIfUserIdExists(req.userId);
        if (!user) {
        ...
        if (height)
            user.height = height;
        if (relationshipTypeSeeking)
            user.relationshipTypeSeeking = relationshipTypeSeeking;
        if (hairColor)
        ...
        if (secondLanguage)
            user.secondLanguage = secondLanguage;
        if (bodyType)
        ...
        if (doesSmoke !== '')
            user.doesSmoke = doesSmoke;
        if (doesDoDrugs !== '')
            user.doesDoDrugs = doesDoDrugs;
        if (doesDrink !== '')
            user.doesDrink = doesDrink;
        if (religion)
            user.religion = religion;
        if (profession)
            user.profession = profession;
        if (doesHavePets !== '')
            user.doesHavePets = doesHavePets;
        if (personality)
            user.personality = personality;
        if (ambitiousness)
            user.ambitiousness = ambitiousness;
        if (datingIntent)
            user.datingIntent = datingIntent;
        if (longestRelationShip)
            user.longestRelationShip = longestRelationShip;
        if (income)
            user.income = income;
        if (doesDateInteracially !== '')
            user.doesDateInteracially = doesDateInteracially;
